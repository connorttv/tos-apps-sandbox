...
Use `SettingsSection` to group related settings under a collapsible heading. Always prefer this over plain `SettingsHeading` — it lets users collapse sections they don't need.

```typescript
import { SettingsSection } from '@telemetryos/sdk/react'
...
- `defaultOpen` (boolean, default `true`) — Whether the section starts expanded

```typescript
{/* Start collapsed for rarely-used settings */}
...
Use `SettingsHeading` for non-collapsible headings within a section or at the top level:
...
| `SettingsSection` | Collapsible accordion group (recommended for organizing settings) |
| `SettingsHeading` | Non-collapsible section heading |
...
6. **Not using SettingsSection** - Wrap groups of related settings in `SettingsSection` for collapsible organization
